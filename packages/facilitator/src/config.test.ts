...
describe("loadConfig", () => {
  ...
  it("markerWeightMultiplier defaults to 1.15 when unset", () => {
    minimal();
    delete process.env.CANTON_X402_MARKER_WEIGHT_MULTIPLIER;
    expect(loadConfig().markerWeightMultiplier).toBe(1.15);
  });
  ...
  it("markerWeightMultiplier falls back to 1.15 on a non-numeric / non-positive value", () => {
    minimal();
  ...
  it("markerMaxWeightPerRound defaults to 1000; honors a valid override", () => {
    minimal();
    delete process.env.CANTON_X402_MARKER_MAX_WEIGHT_PER_ROUND;
    expect(loadConfig().markerMaxWeightPerRound).toBe(1000);
    process.env.CANTON_X402_MARKER_MAX_WEIGHT_PER_ROUND = "2500";
    expect(loadConfig().markerMaxWeightPerRound).toBe(2500);
    process.env.CANTON_X402_MARKER_MAX_WEIGHT_PER_ROUND = "nope";
    expect(loadConfig().markerMaxWeightPerRound).toBe(1000);
  });
