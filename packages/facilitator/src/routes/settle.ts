...
export async function registerSettleRoute(
  ...
  app.post(
    ...
    async (req, reply): Promise<SettleResponse | { error: string }> => {
      ...
      // Rate-limit per submission-ref AND per client IP, plus globally (after
      // shape, so malformed bodies do not consume quota). The wire `payer` claim
      // is gone, so this bucket keys on the relay stash reference: it bounds a
      // caller HAMMERING a single submission ref. Per-payer VOLUME is already
      // bounded UPSTREAM at prepare-time by tfPay.capPerPayer (a payer cannot
      // mint unbounded refs), so a per-ref settle cap is the right residual gate
      // here. CRITICAL: the /settle caller is the MERCHANT (the merchant calls
      ...
      const refKey = `ref:${
        body.paymentPayload?.payload?.submissionRef ?? "unknown"
      }`;
      const ipKey = `ip:${clientIp(req)}`;
      const ipCap = svc.settleRateLimit?.maxPerIp ?? 0;
      if (!limiter.allowKeys([refKey, { key: ipKey, max: ipCap }], nowMs)) {
        req.log.warn(
          { ref: refKey, ip: ipKey },
          "settle rate-limited"
        );
      ...
      if (v.method === "transfer-factory") {
        ...
        try {
          ...
          // A ledger ITR_InsufficientFunds rejection gets its own named code (the
          // pinned input holdings no longer cover the amount); every other execute
          // failure stays the generic execute_failed.
          if (isInsufficientFundsError(err)) {
            return failed("invalid_exact_canton_insufficient_balance");
          }
