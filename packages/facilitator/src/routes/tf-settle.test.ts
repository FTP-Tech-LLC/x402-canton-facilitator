...
function tfBody(
  over: Partial<{ amount: string; payTo: string; memo: string }> = {}
): FacilitatorRequest {
  return {
    x402Version: 2,
    paymentPayload: {
...
describe("transfer-factory /verify arm (runValidation)", () => {
  ...
  it("legacy tolerance: a payload that STILL carries a `payer` (old 0.6.x client) validates + settles like one without — the wire claim is IGNORED, the stash-proven payer wins", async () => {
    const stash = makeStash();
    await seedRow(stash); // stash row payer = PAYER (the PROVEN payer)
    // An old client still emits `payer`, and even claims to be someone else.
    // The facilitator ignores the wire field entirely (loose object bypasses
    // the type so we can inject it).
    const body = tfBody();
    (body.paymentPayload.payload as Record<string, unknown>).payer =
      "attacker::claims-to-be-someone-else";
    const v = await runValidation(
      body,
      { facilitatorParty: FACILITATOR, network: "canton:mainnet", tf: { stash, tfEnabled: true } } as never,
      Date.now()
    ...
    expect(v.ok).toBe(true);
    // The proven payer is the stash row's, NEVER the wire claim.
    if (v.ok) expect(v.payer).toBe(PAYER);
    // …and it settles exactly like a payer-less payload.
    const r = await callSettle(baseSvc(stash), body);
    expect(r.success).toBe(true);
    if (r.success) expect(r.payer).toBe(PAYER);
  });

  it("memo required + stash carries the MATCHING memo → valid", async () => {
    const stash = makeStash();
    await seedRow(stash, { memo: "order-42" });
    const v = await runValidation(
      tfBody({ memo: "order-42" }),
      { facilitatorParty: FACILITATOR, network: "canton:mainnet", tf: { stash, tfEnabled: true } } as never,
      Date.now()
    );
    expect(v.ok).toBe(true);
  ...
  it("memo required but the stash row has NO memo → memo_mismatch (fail-closed)", async () => {
    const stash = makeStash();
    await seedRow(stash); // old / non-compliant client dropped the memo
    const v = await runValidation(
      tfBody({ memo: "order-42" }),
      { facilitatorParty: FACILITATOR, network: "canton:mainnet", tf: { stash, tfEnabled: true } } as never,
      Date.now()
    ...
    if (!v.ok) expect(v.reason).toBe("invalid_exact_canton_memo_mismatch");
  });

  it("memo required + stash carries a DIFFERENT memo → memo_mismatch", async () => {
    const stash = makeStash();
    await seedRow(stash, { memo: "order-99" });
    const v = await runValidation(
      tfBody({ memo: "order-42" }),
      { facilitatorParty: FACILITATOR, network: "canton:mainnet", tf: { stash, tfEnabled: true } } as never,
      Date.now()
  ...
  it("merchant sets NO memo → no check even if the stash carries one", async () => {
    const stash = makeStash();
    await seedRow(stash, { memo: "payer-private-note" });
    const v = await runValidation(
      tfBody(), // no extra.memo required
      { facilitatorParty: FACILITATOR, network: "canton:mainnet", tf: { stash, tfEnabled: true } } as never,
      Date.now()
    );
    expect(v.ok).toBe(true);
  ...
  it("T9: receiver mismatch → merchant_mismatch", async () => {
    ...
    // A mismatch outcome echoes the STASH row's proven payer (not a wire claim).
    if (!v.ok) expect(v.payer).toBe(PAYER);
  });
...
describe("transfer-factory /settle arm", () => {
  ...
  it("execute rejected with ITR_InsufficientFunds → insufficient_balance (named)", async () => {
    const stash = makeStash();
    await seedRow(stash);
    const svc = baseSvc(stash, {
      execute: async () => {
    ...
    expect(r.success).toBe(false);
    if (!r.success)
      expect(r.errorReason).toBe("invalid_exact_canton_insufficient_balance");
    // No funds moved — the settle idempotency record is NOT written.
    expect((await stash.get("REF"))!.settledUpdateId).toBeUndefined();
  });

  it("execute rejected with a NON-ITR_InsufficientFunds error → generic execute_failed", async () => {
    const stash = makeStash();
    await seedRow(stash);
    const svc = baseSvc(stash, {
      execute: async () => {
        // A superficially similar marker that is NOT the insufficient-funds one.
        throw new Error("ITR_SomeOtherInterpretationError: unrelated rejection");
      },
    });
    const r = await callSettle(svc, tfBody());
    expect(r.success).toBe(false);
