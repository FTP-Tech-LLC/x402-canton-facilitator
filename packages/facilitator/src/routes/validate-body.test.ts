...
describe("validateFacilitatorRequestShape", () => {
  ...
  // PAYER FIELD: the wire payload no longer carries a `payer` — it was an
  // untrusted client claim, removed per spec. The body validator neither
  // requires nor rejects it: an absent payer is valid, and a legacy 0.6.x client
  // that still sends one is accepted (the field is simply IGNORED).
  describe("payer field (removed — ignored, never required)", () => {
    ...
    it("accepts a payload that STILL carries a legacy `payer` (ignored)", () => {
    ...
    it("accepts even an empty-string `payer` (ignored, not validated)", () => {
      const r = validateFacilitatorRequestShape(
        withPayload({ ...base, payer: "" })
      );
      expect(r.ok).toBe(true);
    });

    it("accepts a legacy `payerParty` alongside `payer` (both retired keys ignored)", () => {
      const r = validateFacilitatorRequestShape(
  ...
  it("accepts a payload with NO `payer` and only a (retired, empty) payerParty", () => {
    const body = {
      ...VALID,
      paymentPayload: {
        ...VALID.paymentPayload,
        // `payer` is ABSENT and the only payer-party field is the empty (retired)
...
describe("validate-body additional edge cases", () => {
  ...
  it("accepts a null (retired) payerParty when `payer` is absent — neither is validated", () => {
    const body = {
      ...VALID,
      paymentPayload: {
        ...VALID.paymentPayload,
