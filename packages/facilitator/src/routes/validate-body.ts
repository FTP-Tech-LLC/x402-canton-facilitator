...
export function validateFacilitatorRequestShape(
  ...
  // The wire payer-party field is `payer` (aligns with SettleResponse/
  // VerifyResponse `payer`) — require it present + non-empty.
  if (typeof inner.payer !== "string" || inner.payer.length === 0) {
    return {
      ok: false,
      error: "paymentPayload.payload.payer required",
    };
  }
